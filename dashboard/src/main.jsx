...
function Settings({ csrf, notify }) {
  ...
  async function save(){setBusy('save');try{const result=await api('/admin/v1/server/settings',{method:'PUT',body:{...form,random_bots:Number(form.random_bots),max_added_bots:Number(form.max_added_bots),player_limit:Number(form.player_limit),new_character_level:Number(form.new_character_level),xp_rate:Number(form.xp_rate),reputation_rate:Number(form.reputation_rate),loot_rate:Number(form.loot_rate),money_rate:Number(form.money_rate),honor_rate:Number(form.honor_rate),profession_skill_rate:Number(form.profession_skill_rate),auction_house_items_per_cycle:Number(form.auction_house_items_per_cycle),temperature:Number(form.temperature),max_tokens:Number(form.max_tokens)}},csrf);notify(result.world_restarted?'Settings applied; worldserver restarted':'Settings applied')}catch(e){notify(e.message,true)}finally{setBusy('')}}
  ...
  return <div className="page"><header className="page-head"><div><p className="eyebrow">Validated configuration</p><h2>Server settings</h2></div></header>
    <div className="settings-grid"><section className="panel form-panel"><div className="section-title"><h3>Realm</h3><span>Game settings</span></div><label>Realm name<input value={form.realm_name} onChange={e=>set('realm_name',e.target.value)} maxLength="32"/></label><label>Server type<select value={form.realm_type} onChange={e=>set('realm_type',e.target.value)}><option value="normal">Normal (PvE)</option><option value="pvp">PvP</option><option value="rp">Roleplaying (PvE)</option><option value="rp_pvp">Roleplaying PvP</option></select></label><label>Random bots<input type="number" min="0" max="2000" value={form.random_bots} onChange={e=>set('random_bots',e.target.value)}/></label><label>Maximum added companions<input type="number" min="1" max="80" value={form.max_added_bots} onChange={e=>set('max_added_bots',e.target.value)}/></label><label>New character boost level<input type="number" min="1" max="80" value={form.new_character_level} onChange={e=>set('new_character_level',e.target.value)}/></label><label>Player limit<input type="number" min="1" max="1000" value={form.player_limit} onChange={e=>set('player_limit',e.target.value)}/></label><p className="hint">New characters created after applying this setting start at that level; it does not alter existing characters, gear, skills, or attunements. 55 is a practical endgame-quest starting point. Applying realm, population, or new-character settings safely restarts the worldserver.</p></section>
