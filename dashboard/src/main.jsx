...
function AddonPage() {
  return <div className="page"><header className="page-head"><div><p className="eyebrow">No more command typing</p><h2>Soulforge Commander</h2></div><a className="button primary" href="/admin/v1/addon/download">Download addon</a></header><section className="world-hero"><div><span className="kicker">WoW 3.3.5a · Interface 30300</span><h3>A Mass Effect-style command wheel.</h3><p>Hold one mapped button, move the mouse toward Follow, Stay, Flee, Attack, Tank Pull, Rebuff, or Reset, then release to activate it.</p></div></section><section className="panel"><h3>Install once</h3><ol><li>Download and extract <code>SoulforgeCommander.zip</code>.</li><li>Place <code>SoulforgeCommander</code> in <code>Interface/AddOns</code> inside your legal 3.3.5a client.</li><li>Restart Wow.exe and enable the addon on the character screen.</li><li>Open WoW’s Key Bindings, find <strong>Soulforge Commander</strong>, and map <strong>Hold command wheel</strong>.</li><li>Hold the button, move toward a command, and release. The mouse wheel changes the target scope; direct mouse clicks also work.</li></ol><p className="hint">The addon sends only user-initiated Playerbots commands. It never contacts the AI service and never changes a binding without you.</p></section></div>
}
