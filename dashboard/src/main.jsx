...
function Settings({ csrf, notify }) {
  ...
  async function save(){setBusy('save');try{const result=await api('/admin/v1/server/settings',{method:'PUT',body:{...form,random_bots:Number(form.random_bots),max_added_bots:Number(form.max_added_bots),player_limit:Number(form.player_limit),temperature:Number(form.temperature),max_tokens:Number(form.max_tokens)}},csrf);notify(result.world_restarted?'Settings applied; worldserver restarted':'Settings applied')}catch(e){notify(e.message,true)}finally{setBusy('')}}
