...
function Settings({ csrf, notify }) {
  const [form,setForm]=useState(null), [models,setModels]=useState([]), [newModel,setNewModel]=useState(''), [busy,setBusy]=useState('')
  async function load(){try{const [settings,modelData]=await Promise.all([api('/admin/v1/server/settings'),api('/admin/v1/models')]);setForm(settings);setModels(modelData.models)}catch(e){notify(e.message,true)}}
  useEffect(()=>{load()},[])
  const set=(key,value)=>setForm({...form,[key]:value})
  async function save(){setBusy('save');try{const result=await api('/admin/v1/server/settings',{method:'PUT',body:{...form,random_bots:Number(form.random_bots),max_added_bots:Number(form.max_added_bots),player_limit:Number(form.player_limit),xp_rate:Number(form.xp_rate),reputation_rate:Number(form.reputation_rate),loot_rate:Number(form.loot_rate),money_rate:Number(form.money_rate),honor_rate:Number(form.honor_rate),profession_skill_rate:Number(form.profession_skill_rate),temperature:Number(form.temperature),max_tokens:Number(form.max_tokens)}},csrf);notify(result.world_restarted?'Settings applied; worldserver restarted':'Settings applied')}catch(e){notify(e.message,true)}finally{setBusy('')}}
