...
function UsageChart({ series = [] }) {
  const width = 800, height = 220, padX = 42, padY = 24
  const values = series.flatMap(point => [Number(point.input_tokens || 0), Number(point.output_tokens || 0)])
  const maximum = Math.max(...values, 1)
  const x = index => padX + index * (width - padX * 2) / Math.max(series.length - 1, 1)
  const y = value => height - padY - Number(value || 0) / maximum * (height - padY * 2)
  const points = key => series.map((point, index) => `${x(index)},${y(point[key])}`).join(' ')
  const active = series.some(point => point.requests)
  const first = series[0]?.bucket ? new Date(series[0].bucket).toLocaleTimeString([], { hour: 'numeric' }) : ''
  const last = series.at(-1)?.bucket ? new Date(series.at(-1).bucket).toLocaleTimeString([], { hour: 'numeric' }) : ''
  return <section className="panel usage-chart"><div className="section-title"><h3>Token usage over time</h3><span>hourly · last 24 hours</span></div>{active ? <><div className="chart-legend"><span className="input-line">Input</span><span className="output-line">Output</span></div><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Hourly input and output token usage over the last 24 hours"><line x1={padX} y1={height - padY} x2={width - padX} y2={height - padY} className="chart-axis" /><line x1={padX} y1={padY} x2={padX} y2={height - padY} className="chart-axis" /><polyline points={points('input_tokens')} className="chart-line chart-input" /><polyline points={points('output_tokens')} className="chart-line chart-output" /></svg><div className="chart-labels"><span>{first}</span><span>{tokens(maximum)} token peak</span><span>{last}</span></div></> : <p className="muted">Token history will appear after the first AI request.</p>}</section>
}
...
function AddonPage() {
  return <div className="page"><header className="page-head"><div><p className="eyebrow">No more command typing</p><h2>Soulforge Commander</h2></div><a className="button primary" href="/admin/v1/addon/download">Download addon</a></header><section className="world-hero"><div><span className="kicker">WoW 3.3.5a · Interface 30300</span><h3>A Mass Effect-style command wheel.</h3><p>Hold one mapped button, move the mouse toward Assemble, Follow, Stay, Flee, Attack, Tank Pull, Rebuff, or Reset, then release to activate it.</p></div></section><section className="panel"><h3>Install once</h3><ol><li>Download and extract <code>SoulforgeCommander.zip</code>. WoW cannot load the ZIP itself.</li><li>Verify the exact path <code>Interface/AddOns/SoulforgeCommander/SoulforgeCommander.toc</code>. An extra <code>addons</code> or <code>SoulforgeCommander</code> folder prevents discovery.</li><li>Fully close and restart Wow.exe, then enable the addon on the character screen.</li><li>Open WoW’s Key Bindings, find <strong>Soulforge Commander</strong>, and map <strong>Hold command wheel</strong>.</li><li>Open <strong>Companions</strong> on the wheel. The addon synchronizes the active world’s roster through the game server.</li><li>Enable or disable companions, add local names if desired, then choose <strong>Assemble</strong>.</li><li>Hold the button, move toward a command, and release. The mouse wheel changes the target scope.</li></ol><p className="hint">Install it once. Companion changes sync without another download. The addon sends only user-initiated Playerbots commands and never receives AI authority.</p></section></div>
}
