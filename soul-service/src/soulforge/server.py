...
class SoulHandler(BaseHTTPRequestHandler):
    ...
    def _addon_download(self) -> None:
        ...
        if not root.is_dir():
            ...
            return
        companions = self.server.worlds.companions()
        roster = [item.get("name", "") for item in companions if item.get("name")]
        roster_lines = ["-- Generated by the Soulforge server from the active world.",
                        "SoulforgeCommanderCompanions = {"]
        for name in roster:
            escaped = str(name).replace("\\", "\\\\").replace('"', '\\"').replace("\r", "").replace("\n", "")
            roster_lines.append(f'  "{escaped}",')
        roster_lines.extend(["}", ""])
        ...
        with zipfile.ZipFile(buffer, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            for path in sorted(root.iterdir()):
                if path.is_file():
                    contents = generated_roster if path.name == "Companions.lua" else path.read_bytes()
                    archive.writestr(f"SoulforgeCommander/{path.name}", contents)
